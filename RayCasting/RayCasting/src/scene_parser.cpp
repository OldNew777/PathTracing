...
SceneParser::SceneParser(const char *filename) {
    ...
    if (num_lights == 0) {
        printf("WARNING:    No point/direction lights specified\n");
    }
}
...
void SceneParser::parsePerspectiveCamera() {
    ...
    int height = readInt();
    getToken(token);
    assert(!strcmp(token, "canvasDistance"));
    double canvasDistance = readFloat();
    getToken(token);
    assert(!strcmp(token, "focusDistance"));
    double focusDistance = readFloat();
    getToken(token);
    assert(!strcmp(token, "interfereRatio"));
    double interfereRatio = readFloat();
    getToken(token);
    assert (!strcmp(token, "}"));
    camera = new PerspectiveCamera(center, direction, up, width, height, canvasDistance, focusDistance, interfereRatio);
}
...
Material *SceneParser::parseMaterial() {
    ...
    Vector3f objectColor(1.f, 1.f, 1.f), emissionColor(Vector3f::ZERO);
    float refractRate = 0.f, specularRate = 0.f;

    getToken(token);
    assert (!strcmp(token, "{"));
    while (true) {
        getToken(token);
        if (strcmp(token, "objectColor") == 0) {
            objectColor = readVector3f();
        } else if (strcmp(token, "emissionColor") == 0) {
            emissionColor = readVector3f();
        } else if (strcmp(token, "refractRate") == 0) {
            refractRate = readFloat();
        } else if (strcmp(token, "specularRate") == 0) {
            specularRate = readFloat();
    ...
    auto *answer = new Material(textureFilename, objectColor, emissionColor, refractRate, specularRate);
    return answer;
}
...
Transform *SceneParser::parseTransform() {
    ...
    while (true) {
        if (!strcmp(token, "Scale")) {
            ...
            matrix = matrix * Matrix4d::scaling(s[0], s[1], s[2]);
        } else if (!strcmp(token, "UniformScale")) {
            float s = readFloat();
            matrix = matrix * Matrix4d::uniformScaling(s);
        } else if (!strcmp(token, "Translate")) {
            matrix = matrix * Matrix4d::translation(readVector3f());
        } else if (!strcmp(token, "XRotate")) {
            matrix = matrix * Matrix4d::rotateX(DegreesToRadians(readFloat()));
        } else if (!strcmp(token, "YRotate")) {
            matrix = matrix * Matrix4d::rotateY(DegreesToRadians(readFloat()));
        } else if (!strcmp(token, "ZRotate")) {
            matrix = matrix * Matrix4d::rotateZ(DegreesToRadians(readFloat()));
            ...
            matrix = matrix * Matrix4d::rotation(axis, radians);
            getToken(token);
            assert (!strcmp(token, "}"));
